        ...
        if NPC.IsKillable(npc) and not Entity.IsDormant(npc) and Entity.IsAlive(npc) then
            ...
            if targets[npc_id] == nil then
                ...
                targets[npc_id] = {
                    ...
                    0, npc, 0, health -- 0, npc, ATTACK_TARGET#15, HIT_IN#16, HEALTH_PREDICTION#17
                }
            end
            ...
            local attack_time_full = attackTime + attackPoint + attackBackSwing + NPC.GetTimeToFace(myHero, npc)
            ...
            targets[npc_id][17] = HitMaster.PredictHealth(npc, attack_time_full)
            ...
            if targets[npc_id][16] then
              ...
              -- Renderer.DrawText(font, x, y + 5, targets[npc_id][16] - curtime, 1)
            end
            ...
            elseif targets[npc_id][17] <= oneHitDamage then
                ...
                Renderer.DrawFilledRect(x-size, y-size, 2*size, 2*size)
                targets[npc_id][13] = 2
            elseif time_to_above_dead <= attack_time_full and targets[npc_id][17] <= oneHitDamage * 1.75 then
...
function HitMaster.OnProjectile(projectile) 
    ...
    if targets[npc_id] ~= nil and (Entity.IsNPC(target) or NPC.IsHero(target)) then
      local curtime = GameRules.GetGameTime()
      targets[npc_id][15] = target
      targets[npc_id][16] = curtime + HitMaster.RangedHitDelay(npc, target) -- + 1.0
...
function HitMaster.RangedHitDelay(npc, target)
  local attack_time = 0
  local distance = (Entity.GetAbsOrigin(npc) - Entity.GetAbsOrigin(target)):Length() -- + 12 -- HERO_COLLISION/2
...
function HitMaster.HitDelay(source, target)
    ...
    local attack_time = attackTime + attackPoint + attackBackSwing + NPC.GetTimeToFace(source, target) + totalLatency
...
function HitMaster.PredictHealth(source, time)
    local curtime = GameRules.GetGameTime()
    ...
    --     item = HitMaster.targets[i]
      ...
      if Entity.IsNPC(npc) and item[15] == source and Entity.GetHealth(npc) > 0 then
          ...
          if item[16] >= curtime and attacker_time <= time then
...
function HitMaster.GetOneHitDamageVersus(myHero, npc)
    ...
    local damage = NPC.GetDamageMultiplierVersus(myHero, npc) * NPC.GetTrueDamage(myHero) * NPC.GetArmorDamageMultiplier(npc) - 4
